...
/** width:height of the source glyph. Exported so callers that need to lay out
 * or animate a cone (the Run HUD's slide-in strip measures its own clip window)
 * derive the width from the same number this renders with, instead of keeping a
 * second copy of the ratio that can drift. */
export const CONE_ASPECT = 110 / 135;

export const ConeIcon: React.FC<{ size?: number }> = ({ size = 144 }) => (
  <svg width={size * CONE_ASPECT} height={size} viewBox={CONE_VIEWBOX}>
